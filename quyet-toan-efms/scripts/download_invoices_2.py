...
async def process_company(page, company, search_items, auth_token):
    for item in search_items:
        ...
        try:
            ...
            print(f"Error matching placeholder: {e}")
            # Fallback
            await page.locator("input[type='text']").first.fill("")
            await page.locator("input[type='text']").first.fill(val)
            
        await page.wait_for_timeout(500)
        await page.keyboard.press("Enter")
        
        try:
            # Chờ nút tải hóa đơn xuất hiện (icon download của Ant Design)
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Chờ đăng nhập xong và tự vào trang
        print("\n>>> SAU KHI ĐĂNG NHẬP, VUI LÒNG BẤM VÀO MENU 'TRA CỨU HÓA ĐƠN' <<<\n")
        while True:
            if auth_token and "tracking_inv" in page.url:
                print("Phát hiện đã đăng nhập và vào đúng trang!")
                break
